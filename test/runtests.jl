            ...
            context("$(intercept ? "w/" : "w/o") intercept") do
                ...
                for naivealgorithm = [false, true]
                     context(naivealgorithm ? "naive" : "covariance") do
                        # Now fit with Lasso
                        l = fit(LassoPath, X, y, dist, link, λ=g.lambda, naivealgorithm=naivealgorithm, intercept=intercept, cd_tol=eps(), irls_tol=eps())
                        ...
                        @fact l.coefs => roughly(gbeta, sqrt(eps()))
                        @fact l.b0 => roughly(g.a0, sqrt(eps()))
                    end
                end
            end
        end
